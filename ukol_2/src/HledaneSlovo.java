...
public class HledaneSlovo {
    ...
    public boolean stisknutoPismeno(char pismeno) {
        char normalizedPismeno = normalizeChar(pismeno);

        boolean nalezeno = false;

        for (int i = 0; i < slovo.length(); i++) {
            char normalizedSlovoChar = normalizeChar(slovo.charAt(i));
            if (normalizedSlovoChar == normalizedPismeno && odhalenaPismena[i] == '_') {
    ...
    private char normalizeChar(char c) {
        ...
        return normalized.replaceAll("[^a-zA-Z]", "").toLowerCase().charAt(0);
    }
    ...
    public int getMinPokusu() {
        ...
        for (char c : slovo.toLowerCase().toCharArray()) {
            if (c >= 'a' && c <= 'z') {
